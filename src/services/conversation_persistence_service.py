        ...
        if os.path.exists(self.storage_file):
            try:
                with open(self.storage_file, "r", encoding="utf-8") as f:
                    data = json.load(f)
                    logger.debug(
                        f"Loaded {self._count_total_conversations(data)} conversations from {self.storage_file}"
                    )
                    return data
            except Exception as e:
                logger.error(
                    f"Error loading conversations from {self.storage_file}: {e}"
                )
                return {}
        return {}
    ...
    def _save_conversations_sync(self):
        ...
        try:
            with self.lock:
                ...
                logger.debug(
                    f"Saved {self._count_total_conversations(self._conversations)} conversations to {self.storage_file}"
                )
        except Exception as e:
