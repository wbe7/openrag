...
async def test_completion_with_tools(
    ...
    if provider == "openai":
        ...
        await _test_watsonx_completion_with_tools(
            api_key, llm_model, endpoint, project_id
        )
    elif provider == "ollama":
...
async def _test_openai_completion_with_tools(api_key: str, llm_model: str) -> None:
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code != 200:
                logger.info(
                    "max_tokens parameter failed, trying max_completion_tokens instead"
                )
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if token_response.status_code != 200:
                ...
                raise Exception(
                    f"Failed to authenticate with IBM Watson: {error_details}"
                )
