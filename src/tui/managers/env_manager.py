...
class EnvConfig:
    ...
    openrag_documents_path: str = (
        "$HOME/.openrag/documents"  # Primary documents path for compose
    )
    ...
    openrag_data_path: str = (
        "$HOME/.openrag/data"  # Backend data (conversations, tokens, etc.)
    )
...
class EnvManager:
    ...
    def generate_compose_volume_mounts(self) -> List[str]:
        ...
        paths_str = self.config.openrag_documents_paths.replace(
            "$HOME", str(Path.home())
        )
