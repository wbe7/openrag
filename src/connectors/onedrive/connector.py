...
class OneDriveConnector(BaseConnector):
    ...
    CLIENT_SECRET_ENV_VAR = (
        "MICROSOFT_GRAPH_OAUTH_CLIENT_SECRET"  # pragma: allowlist secret
    )
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        logger.debug(
            f"OneDrive connector __init__ called with config type: {type(config)}"
        )
        ...
        self._base_url = config.get(
            "base_url"
        )  # Generic URL field for OneDrive/SharePoint domain
        logger.debug(
            f"OneDrive connector initialized with base_url from config: {self._base_url}"
        )
        ...
        for info in file_infos:
            ...
            if file_id:
                ...
                if info.get("downloadUrl"):
                    logger.debug(
                        f"Cached download URL for file {file_id}: {info.get('name')}"
                    )
    ...
    async def authenticate(self) -> bool:
        ...
        logger.debug(
            f"OneDrive authenticate() called, oauth is None: {self.oauth is None}"
        )
        try:
            if not self.oauth:
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.info(
                        f"_detect_onedrive_url: webUrl from response: {web_url}"
                    )
                    ...
                    if web_url:
                        ...
                        logger.info(
                            f"_detect_onedrive_url: Detected OneDrive URL: {onedrive_url}"
                        )
                        return onedrive_url
                    else:
                        logger.warning(
                            "_detect_onedrive_url: webUrl is empty in response"
                        )
                else:
                    logger.warning(
                        f"_detect_onedrive_url: Failed to get drive info: {response.status_code}, response: {response.text[:500]}"
                    )
    ...
    async def setup_subscription(self) -> str:
        ...
        if not webhook_url:
            logger.warning(
                "No webhook URL configured, skipping OneDrive subscription setup"
            )
        ...
        try:
            if not await self.authenticate():
                raise RuntimeError(
                    "OneDrive authentication failed during subscription setup"
                )
    ...
    async def _extract_onedrive_acl(
        self, file_id: str, file_metadata: Dict
    ) -> DocumentACL:
        """
        ...
        try:
            ...
            if not access_token:
                logger.warning(
                    f"No access token available for ACL extraction: {file_id}"
                )
                return DocumentACL()

            # OneDrive permissions API endpoint
            permissions_url = (
                f"{self._graph_base_url}/me/drive/items/{file_id}/permissions"
            )
    ...
    async def get_file_content(self, file_id: str) -> ConnectorDocument:
        ...
        try:
            if not await self.authenticate():
                raise RuntimeError(
                    "OneDrive authentication failed during file content retrieval"
                )
            ...
            if not file_metadata:
                ...
                if "!" in file_id and file_id.split("!", 1)[1].startswith("s"):
                    ...
                    shares_content = await self._download_via_shares_endpoint(
                        file_id, headers
                    )
                    if shares_content is not None:
                        acl = DocumentACL(
                            owner="", user_permissions={}, group_permissions={}
                        )
                        return ConnectorDocument(
                            id=file_id,
                            filename="Unknown",
            ...
            if len(parts) == 2:
                ...
                if item_id.startswith("s"):
                    ...
                    share_encodings = [
                        ...
                        base64.urlsafe_b64encode(f"u!{file_id}".encode())
                        .decode()
                        .rstrip("="),
                        # Approach 2: Encode just the share token with "s!" prefix
                        base64.urlsafe_b64encode(f"s!{item_id}".encode())
                        .decode()
                        .rstrip("="),
                    ...
                    for i, encoded in enumerate(share_encodings):
                        try:
                            ...
                            logger.debug(
                                f"Trying shares endpoint approach {i + 1}: {url}"
                            )
                            ...
                            if response.status_code == 200:
                                logger.info(
                                    f"Shares endpoint approach {i + 1} succeeded"
                                )
                ...
                logger.info(
                    f"Trying drives endpoint: /drives/{drive_id}/items/{item_id}"
                )
                try:
        ...
        try:
            ...
            if "!" in file_id:
                ...
                if len(parts) == 2:
                    ...
                    if item_id.startswith("s"):
                        content = await self._download_via_shares_endpoint(
                            file_id, headers
                        )
    ...
    async def _download_file_from_url(self, download_url: str) -> bytes:
        ...
        try:
            async with httpx.AsyncClient() as client:
                response = await client.get(
                    download_url, timeout=60, follow_redirects=True
                )
                response.raise_for_status()
                return response.content
        except Exception as e:
    ...
    async def cleanup_subscription(self, subscription_id: str) -> bool:
        ...
        try:
            if not await self.authenticate():
                logger.error(
                    "OneDrive authentication failed during subscription cleanup"
                )
                return False
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code in [200, 204, 404]:
                    logger.info(
                        f"OneDrive subscription {subscription_id} cleaned up successfully"
                    )
                    return True
                else:
                    logger.warning(
            ...
            logger.error(
                f"Failed to cleanup OneDrive subscription {subscription_id}: {e}"
            )
            return False
