...
class SharePointOAuth:
    ...
    async def is_authenticated(self) -> bool:
        ...
        logger.info(
            f"SharePoint is_authenticated: Starting check, token_file={self.token_file}"
        )
        try:
            # First try to load credentials if we haven't already
            if not self._current_account:
                logger.info(
                    "SharePoint is_authenticated: No current account, loading credentials"
                )
                load_result = await self.load_credentials()
                logger.info(
                    f"SharePoint is_authenticated: load_credentials returned {load_result}"
                )
            ...
            if self._current_account:
                ...
                logger.info(
                    f"SharePoint is_authenticated: RESOURCE_SCOPES={self.RESOURCE_SCOPES}"
                )
                ...
                if result and "access_token" in result:
                    ...
                    logger.warning(
                        f"SharePoint is_authenticated: Full result: {result}"
                    )
            ...
            logger.warning(
                f"SharePoint is_authenticated: Fallback also failed, result: {result}"
            )
            return False
    ...
    def get_access_token(self) -> str:
        ...
        try:
            ...
            if self._current_account:
                ...
                if result and "access_token" in result:
                    logger.info(
                        "SharePoint get_access_token: Success with current account"
                    )
            ...
            # Fallback: try without specific account
            logger.info(
                "SharePoint get_access_token: Fallback - trying without account"
            )
            result = self.app.acquire_token_silent(self.RESOURCE_SCOPES, account=None)
            if result and "access_token" in result:
            ...
            logger.error(
                f"SharePoint get_access_token: All attempts failed, error: {error_msg}"
            )
    ...
    def get_access_token_for_resource(self, resource_url: str) -> str:
        ...
        """
        logger.info(
            f"SharePoint get_access_token_for_resource: resource_url={resource_url}"
        )
        ...
        logger.info(
            f"SharePoint get_access_token_for_resource: scopes={sharepoint_scopes}"
        )
        ...
        try:
            ...
            if result and "access_token" in result:
                logger.info(
                    "SharePoint get_access_token_for_resource: Fallback success"
                )
                return result["access_token"]
