...
class SharePointConnector(BaseConnector):
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.info(
                        f"_detect_sharepoint_url: webUrl from response: {web_url}"
                    )
                    ...
                    if web_url:
                        ...
                        logger.warning(
                            "_detect_sharepoint_url: webUrl is empty in response"
                        )
                else:
                    logger.warning(
                        f"_detect_sharepoint_url: Failed to get drive info: {response.status_code}, response: {response.text[:500]}"
                    )
    ...
    async def _extract_sharepoint_acl(
