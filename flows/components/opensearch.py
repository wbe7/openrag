...
class OpenSearchVectorStoreComponentMultimodalMultiEmbedding(LCVectorStoreComponent):
    ...
    icon: str = "OpenSearch"
    description: str = "Store and search documents using OpenSearch with multi-model hybrid semantic and keyword search."
    ...
    def _ensure_embedding_field_mapping(
        ...
        try:
            ...
            logger.warning(
                f"Could not add embedding field mapping for {field_name}: {e}"
            )
            raise
    ...
    def build_vector_store(self) -> OpenSearch:
        ...
        if not has_search_query:
            logger.debug(
                "Ingestion-only mode activated: search operations will be skipped"
            )
    ...
    def _add_documents_to_vector_store(self, client: OpenSearch) -> None:
        ...
        logger.debug(
            f"[INGESTION] Valid embeddings after filtering: {len(embeddings_list)}"
        )
